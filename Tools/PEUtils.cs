...
namespace PhysicalEngine.Tools
{
    public static class PEUtils
    {
        ...
        {
            ratio = PEMathTools.Clamp(ratio, 0.25f, 1f);
        ...
        public static Vector2 Transform(Vector2 value, PETransform transform)
        {
            return new Vector2(
        ...
        public static Vector2 Transform(float x, float y, PETransform transform)
        {
            return new Vector2(
        ...
        public static bool IntersectBoxes(PEBox a, PEBox b)
        {
            if( a.Max.X <= b.Min.X || 
                b.Max.X <= a.Min.X ||
                a.Max.Y <= b.Min.Y || 
        ...
        public static bool IntersectCircles(in PECircle a, in PECircle b)
        {
        ...
        public static bool IntersectBoxes(PEBox a, PEBox b, out Vector2 mtv)
        {
            mtv = Vector2.Zero;

            if(!PEUtils.IntersectBoxes(a, b))
            {
                return false;
            }
        ...
        public static bool IntersectSegments(Vector2 a, Vector2 b, Vector2 c, Vector2 d, out Vector2 ip)
        {
            ...
            if(!PEUtils.IntersectSegmentsFast(a.X, a.Y, b.X, b.Y, c.X, c.Y, d.X, d.Y))
            {
                return false;
            }
        ...
        public static bool IntersectSegments(Vector2 a, Vector2 b, Vector2 c, Vector2 d)
        {
            ...
            float c1 = PEMathTools.Cross(ab, ac);
            float c2 = PEMathTools.Cross(ab, ad);

            float c3 = PEMathTools.Cross(cd, ca);
            float c4 = PEMathTools.Cross(cd, cb);
        ...
        public static bool Intersect(in PECircle circle, in PEBox box)
        {
            float cx = PEMathTools.Clamp(circle.Center.X, box.Min.X, box.Max.X);
            float cy = PEMathTools.Clamp(circle.Center.Y, box.Min.Y, box.Max.Y);

            Vector2 cp = new Vector2(cx, cy);

            return PEMathTools.Distance(circle.Center, cp) < circle.Radius;
        }
        ...
        public static bool IntersectCircles(PECircle a, PECircle b, out float depth, out Vector2 normal)
        {
            depth = 0f;
            normal = Vector2.Zero;
        ...
        public static void CreateBoundingBox(Vector2[] vertices, int[] hull, out PEBox box)
        {
            float xmin = float.MaxValue;
            float xmax = float.MinValue;
            float ymin = float.MaxValue;
            ...
            box = new PEBox(xmin, xmax, ymin, ymax);
        }

        public static void CreateBoundingBox(Vector2[] vertices, out PEBox box)
        {
            float xmin = float.MaxValue;
            float xmax = float.MinValue;
            float ymin = float.MaxValue;
        ...
        public static bool IsPointInTriangle(Vector2 p, Vector2 a, Vector2 b, Vector2 c)
        {
            ...
            float c1 = PEMathTools.Cross(ab, ap);
            float c2 = PEMathTools.Cross(bc, bp);
            float c3 = PEMathTools.Cross(ca, cp);
        ...
        {
            return MathF.Abs(a - b) <= PEUtils.VerySmallAmount;
        }

        public static bool IsNearlyEqual(Vector2 a, Vector2 b)
        {
            return
                (MathF.Abs(a.X - b.X) <= PEUtils.VerySmallAmount) && 
                (MathF.Abs(a.Y - b.Y) <= PEUtils.VerySmallAmount);
        }
    }
}